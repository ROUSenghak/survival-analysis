...
def verify_overlay_archive(archive: Path, manifest_path: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="boamp-overlay-") as tmp:
        ...
        expected_members = sorted([source_manifest_rel, *[row["path"] for row in source_state["files"]]])
        ...
        for row in source_state["files"]:
            path = root / row["path"]
            if bool(row["exists"]) and bool(row["is_file"]):
                if not path.exists():
                ...
                mismatches.append(
                    {
                        "path": row["path"],
                        "expected_absent_or_nonfile": True,
                        "actual_exists": True,
                    }
                )
    ...
    status = (
        "PASS"
        if archive_sha_matches_manifest
    ...
    return {
        ...
        "n_hashed_paths_checked": len(source_state["files"]),
