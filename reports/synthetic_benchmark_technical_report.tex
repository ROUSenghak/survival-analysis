...
What it does not yet support is a defensible \emph{comparison} of linkage algorithms. One critical gate fails: the saved parameters do not reproduce the dataset, so a result obtained here cannot be regenerated from version control alone. Only one world exists per scenario, so no result can be separated from its seed. Buyer-activity concentration and internal text duplication are both materially below the real corpus, and both make linkage easier than it should be. Accordingly, \texttt{READY\_FOR\_LINKAGE} should be read as \emph{ready to begin controlled algorithm experiments}, not as \emph{ready to rank algorithms}.
...
\begin{enumerate}
    ...
    \item \textbf{Candidate tail is lighter than real.} v0.3 passes p75/p90/p95 gates, but real p90 and p95 counts remain higher than synthetic values.
    \item \textbf{No score calibration has been done yet.} The benchmark is ready for running linkage algorithms, not for claiming final linkage performance.
    \item \textbf{The dataset is not reproducible from its scenario file.} The parameters actually used exist only in the run manifest. This is a release-blocking defect and is the first thing to fix.
    \item \textbf{Buyer activity is too evenly spread.} Gini 0.56 against a real 0.83. Dense candidate neighbourhoods around highly active buyers are under-represented, so the hardest linkage cases are under-tested.
    \item \textbf{Internal text duplication regressed in v0.2.} 9.0\% of synthetic objects are non-unique against 51.4\% in real \boamp{}. Text carries more discriminating signal here than it does in reality.
    \item \textbf{Only one seed exists per scenario.} No reported quantity can currently be separated from its particular world draw, so seed-stability claims are unsupportable and are reported \textsc{Inconclusive}.
...
The final machine-readable readiness gate is
\nolinkurl{reports/tables/synthetic_benchmark/v0_3_temporal_candidate_revision/temporal_candidate_freeze_gate.csv}.
...
The correct next step is to run linkage algorithms against the v0.3 observed synthetic notices, store their predicted links, and evaluate them against the hidden truth relations. The evaluation should report candidate coverage, ranking quality, precision/recall against hidden truth, threshold sensitivity, and failure cases by buyer-key type, \cpv{} division, text pattern, and temporal gap.

Before that evaluation can support algorithm \emph{comparisons}, four things should be fixed in order. First, write the swept v0.3 parameters back into a versioned scenario file so the dataset regenerates from version control, which clears the only failing critical gate. Second, generate at least a second world seed for the central scenario so seed stability stops being unmeasurable. Third, restore the buyer-activity concentration and the internal text-duplication rate, both of which currently make linkage easier than reality. Fourth, decide explicitly whether the 60-month runway and candidate upper tail are being fixed or being accepted into the fidelity budget.

Until that evaluation is complete, the defensible claim is:
\begin{quote}
Within declared observable tolerances and a documented scenario envelope, the synthetic benchmark reproduces selected \boamp{} structures and candidate difficulties; its hidden truth is internally coherent, does not leak, and contains no copied real records; and conclusions about linkage algorithms must still be reported with sensitivity to the properties that remain unidentified, to the single available seed, and to the specification defect that currently blocks exact regeneration.
\end{quote}
