...
precision **0.719**, end-to-end recall **0.235**,
and end-to-end pair-F1 **0.354** across the held-out evaluation grid.

The result is not mainly a "which classifier can separate pairs" story. It is a pipeline story:
the production candidate generator exposes only **10,333** of **32,404**
in-scope truth links to scoring, a weighted blocking recall of **31.9%**
and an unweighted world-average recall of **0.291**. This is why gradient boosting has
fixed-candidate F1 **0.728** but end-to-end F1 only
**0.354**.

Logistic regression recovers essentially the same number of true links as gradient boosting
(7,704 versus 7,624), but accepts far more
links (22,706 versus 10,610). The extra volume lowers
precision from **0.719** to **0.339**. The current weighted
...
(0.238 versus 0.235), but gradient boosting
...
enter the candidate set. The evaluation run scored **879,179** candidate pairs over
**32,404** in-scope truth links, but only **10,333** truth links were
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| central_provisional | 4 | 80,801 | 6,488 | 2,166 | 0.334 | 0.303 | 0.355 |
...
| --- | --- | --- | --- | --- | --- |
| central_provisional | Gradient boosting | 0.382 | 0.770 | 0.254 | 0.015 |
| central_provisional | Logistic regression | 0.350 | 0.549 | 0.258 | 0.011 |
| central_provisional | Current weighted composite | 0.222 | 0.204 | 0.244 | 0.014 |
| central_provisional | Fellegi-Sunter style | 0.168 | 0.181 | 0.158 | 0.012 |
...
For gradient boosting, end-to-end recall is **0.544**
when a source has exactly one candidate, but only **0.120**
...
**0.285** for `RAW_SIRET` truth links versus
**0.223** for `NAME_FALLBACK`. CPV availability
matters as well: recall is **0.251** when CPV is present
and **0.099** when CPV is missing.
...
| Logistic regression | logistic_score | 0.745678 | calibration F1 maximum |
| Gradient boosting | gradient_boosting_score | 0.230964 | calibration F1 maximum |
