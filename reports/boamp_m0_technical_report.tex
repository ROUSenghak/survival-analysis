...
\date{Current generated results: repository state of July 2026}
...
be: Section~\ref{sec:scope} gives the business framing; Sections
\ref{sec:data}--\ref{sec:dq} document the raw data and its quality;
Sections \ref{sec:pipeline}--\ref{sec:output} document preprocessing and the
M0 linking algorithm mechanically, with a full worked example in
Section~\ref{sec:worked}; Section~\ref{sec:reliability} consolidates the
credibility diagnostics (blocking coverage, score margins, an unsupervised
Fellegi--Sunter precision/recall estimate, corruption-recovery robustness);
Sections \ref{sec:survival-methods}--\ref{sec:sensitivity} document the
survival analysis, its methods, results, and robustness under alternative
event definitions; Section~\ref{sec:conclusions} states plainly what is and
is not supported by the current evidence. All figures and tables reflect the
...
\textbf{Text similarity} is TF-IDF cosine similarity between source and
...
$\hat e$. The best-scoring candidate was \texttt{19-141668}, published
...
(division 35, class 3511, category 35110).
...
$0.4286 \geq \tau_{\text{balanced}}=0.3230$, the source is recorded as
\texttt{event}=1 with \texttt{time\_to\_event\_or\_censor\_months}=48.29.
...
\label{sec:fs}
An unsupervised two-class Beta mixture (match class $M$, non-match class
$U$) was fitted by EM over the comparison vectors
...
\textbf{Kaplan--Meier estimator.} $\hat S(t) = \prod_{j:t_j\leq t}(1 -
d_j/n_j)$, the probability of not-yet-linked survival past $t$.

\textbf{Restricted mean survival time (RMST)} at horizon $\tau=60$ months,
$\int_0^{\tau}\hat S(t)\,dt$, is used as the primary summary statistic
...
\textbf{Cox proportional hazards model}: $h(t\mid \mathbf x_i) = h_0(t)
\exp(\boldsymbol\beta^\top \mathbf x_i)$, fit with \texttt{lifelines}
(penalizer $=0.01$), with covariates log(1 + declared duration months),
...
\textbf{Proportional-hazards diagnostic}: Schoenfeld-residual test
($H_0$: PH holds) via \texttt{lifelines}' \texttt{check\_assumptions}.
...
across variants: division 32 (telecommunications equipment) has the
highest RMST at 60 months under broad (54.7 mo) but is not the highest
under strict, while division 72 (IT services) --- the largest single
division by volume (889 sources) --- ranks consistently mid-to-low. A
...
window (HR $>1$). This is strong evidence that declared duration should
...
the lower quintiles, so the model carries \emph{some} ranking signal even
in a design known to have duration-leakage concerns, but calibration is
uneven (quintiles 1--3 under-predict relative to their already-small
observed rates; quintile 5 under-predicts the true rate by roughly 8.5
points). \textbf{This is calibration against the constructed BOAMP proxy
...
\end{verbatim}
Notebook front-ends for the audit/robustness steps are provided in
\texttt{notebooks/13\_*} through \texttt{notebooks/16\_*}; each has a
\texttt{RUN\_SCRIPT} toggle (default: read existing outputs; set true to
regenerate). Each script is idempotent with respect to \texttt{data/raw}
(raw files are never overwritten) and logs to
\texttt{reports/run\_logs/run\_log.md}.
