...
\title{BOAMP Renewal Linkage --- Layer 1 (\texttt{boamp\_only}, formerly M0)\\
...
\subsection{End-to-End Preprocessing and Survival-Dataset Construction}
\label{sec:endtoend-construction}
This subsection is a roadmap only; detailed formulas are kept in the dedicated
preprocessing, linkage, and survival sections below to avoid repeating the same
rules in multiple places. Let the raw BOAMP collection be
\[
\mathcal{R}=\{r_1,\ldots,r_N\}, \qquad N=84{,}623.
\]
The parser maps each nested raw notice to a flattened tabular row
\[
F_{\mathrm{flat}}:\mathcal{R}\rightarrow\mathcal{X},
\qquad r_i\mapsto x_i,
\]
where $\mathcal{X}$ is a mixed feature space containing dates, identifiers,
categorical fields, and text. The preprocessing script then constructs
\[
z_i=\bigl(\operatorname{id}_i,b_i,t_i,a_i,d_i,\hat e_i,\kappa_i,o_i,q_i\bigr),
\]
where $b_i$ is the buyer key, $t_i$ the publication date, $a_i$ the refined
start date, $d_i$ the observed or imputed duration, $\hat e_i=a_i+d_i$ the
estimated end date, $\kappa_i$ the CPV hierarchy, $o_i$ the normalized object
text, and $q_i$ quality/provenance flags.

The source set used for recurrence construction is
\[
\mathcal{S}=
\{i:\texttt{notice\_type}_i=\texttt{APPEL\_OFFRE},\;
\texttt{digital}_i=1,\; b_i\neq\texttt{MISSING}\},
\qquad |\mathcal{S}|=3{,}159.
\]
The rest of the report uses this notation: Section~\ref{sec:pipeline}
defines $b_i$, $a_i$, $d_i$, $\hat e_i$, $\kappa_i$, and $o_i$;
Section~\ref{sec:algo} defines the blocked pair universe, comparison scores, and
accepted proxy-recurrence links; Section~\ref{sec:output} defines the final
survival handoff. In compact form, the persisted pipeline is
\[
\begin{aligned}
84{,}623\ \text{cleaned notices}
&\longrightarrow 3{,}159\ \text{digital AO sources}\\
&\longrightarrow 6{,}137\ \text{blocked pairs}\\
&\longrightarrow 618\ \text{balanced accepted links}\\
&\longrightarrow 3{,}159\ \text{survival rows}.
\end{aligned}
\]
...
\midrule
Buyer identity & 27.2\% of all notices carry a format-valid raw SIRET (99.6\% of those also pass checksum). & Use \texttt{SIRET:\ldots} when a format-valid 14-digit SIRET exists; else \texttt{SIREN:\ldots} when a format-valid 9-digit SIREN exists; else \texttt{NAME:\ldots} from a normalized buyer name. \texttt{buyer\_key\_type} records which rule fired. No row is ever dropped for missing buyer identity --- 0\% \texttt{MISSING} across all 84{,}623 notices. \\
...
\label{sec:buyerkey}
For notice $i$, let $s_i^{(14)}$ be the first format-valid raw 14-digit SIRET
candidate recovered from BOAMP, let $s_i^{(9)}$ be the first format-valid raw
...
\end{cases}
\end{equation}
The companion \texttt{buyer\_key\_type} records which branch fired:
\[
\texttt{buyer\_key\_type}_i \in
\{\texttt{RAW\_SIRET},\texttt{RAW\_SIREN},
\texttt{NAME\_FALLBACK},\texttt{MISSING}\}.
\]
The function $\phi(\cdot)$ normalizes the buyer name by lowercasing, stripping
accents, removing punctuation noise, and collapsing whitespace. This is purely
internal cleaning of BOAMP-provided values, never an external lookup. Checksum
validity is recorded separately for audit, while the key-selection branch uses
format-valid identifiers. When a valid SIRET exists, the script also derives
\texttt{buyer\_siren\_clean} from its first nine digits for diagnostics, but
the key remains \texttt{SIRET:\ldots} because SIRET has priority. Across the
3{,}159 M0 sources: 713 (22.6\%) resolve to \texttt{RAW\_SIRET}, 0 to
\texttt{RAW\_SIREN}, 2{,}446 (77.4\%) to \texttt{NAME\_FALLBACK}, and 0 to
\texttt{MISSING}.
...
\subsection{Notation}
The notation follows Section~\ref{sec:endtoend-construction}. Let
$i\in\mathcal{S}$ index an eligible digital/ICT source notice and let
$j\in\mathcal{S}$ index a possible later candidate. The source carries buyer
key $b_i$, publication date $t_i$, refined start date $a_i$, duration $d_i$,
estimated end date $\hat e_i=a_i+d_i$, CPV hierarchy $\kappa_i$, normalized
object text $o_i$, and buyer-key provenance. The blocked candidate set for
source $i$ is denoted $\mathcal{C}_W(i)$.
...
universe is
\begin{equation}
\Omega_W = \big\{(i,j)\in\mathcal{S}\times\mathcal{S} : b_j=b_i,\ t_j>t_i,\ |t_j-\hat e_i|\leq W \big\},
\end{equation}
with source-specific candidate set
\begin{equation}
\mathcal{C}_W(i)=\{j:(i,j)\in\Omega_W\}.
\end{equation}
The subscript is omitted when $W=6$ months. The set is capped, per source,
at the 30 temporally nearest candidates
...
$\Omega_6$ contains 6{,}137 pairs covering 1{,}236 of the 3{,}159 eligible
...
\begin{equation}
s_{\text{time}}(i,j) = \mathrm{clip}\!\left(1 - \frac{|t_j-\hat e_i|}{W},\ 0,\ 1\right).
\end{equation}
This form is consistent with the implemented blocking rule and remains valid
when the refined start date $a_i$ differs from the publication date $t_i$.
Writing $|(t_j-t_i)-d_i|$ would be equivalent only in the fallback case
$a_i=t_i$.
...
Let
$\boldsymbol\gamma_{ij}=(s_{\text{text}}(i,j),s_{\text{cpv}}(i,j),
s_{\text{time}}(i,j),s_{\text{buyer}}(i))^{\top}$ and
$\boldsymbol w=(0.35,0.30,0.25,0.10)^{\top}$. Then
\begin{equation}
S(i,j)=\boldsymbol w^{\top}\boldsymbol\gamma_{ij}
=0.35\,s_{\text{text}}(i,j)+0.30\,s_{\text{cpv}}(i,j)
+0.25\,s_{\text{time}}(i,j)+0.10\,s_{\text{buyer}}(i)\in[0,1].
\end{equation}
...
For each source with $\mathcal{C}_W(i)\neq\varnothing$, define
\begin{equation}
j^{\star}(i)=\underset{j\in\mathcal{C}_W(i)}{\arg\max}\;S(i,j).
\end{equation}
The rank-1 candidate's score is retained. Three link \emph{variants}
...
\texttt{balanced} $\subset$ \texttt{broad}. For any variant $v$, define
\begin{equation}
\delta_i^{(v)}=
\mathbf{1}\!\left\{
\mathcal{C}_W(i)\neq\varnothing
\quad \text{and}\quad
S\bigl(i,j^{\star}(i)\bigr)\geq\tau_v
\right\}.
\end{equation}
The reference file stores
$\texttt{event}_i=\delta_i^{(\mathrm{balanced})}$; below, the variant
superscript is omitted when the balanced definition is understood.
...
\texttt{m0\_composite\_score} & float & $S(i,j^{\star}(i))$ for linked rows; empty when \texttt{event}=0. \\
...
\midrule
$t_j-t_i$ & publication-date gap used as observed event time & 48.29 \\
$t_j-\hat e_i$ & candidate date minus estimated end date & $-2.595$ \\
$|t_j-\hat e_i|$ & absolute temporal deviation & 2.595 \\
$s_{\text{time}}$ & $1-2.595/6$ & 0.5675 \\
...
\end{tabular}
\end{table}
The two time quantities serve different roles. The 48.29-month publication
gap is the stored survival-event time, whereas the 2.595-month distance from
$\hat e_i$ determines the temporal similarity used for linkage.
...
For the balanced reference variant, suppress the superscript $(v)$ introduced
in Section~\ref{sec:endtoend-construction}. Let $T_i$ denote time to the
constructed proxy-recurrence event and $C_i$ the censoring time. The observed
survival outcome is
\begin{equation}
T_i^{\mathrm{obs}}=\min(T_i,C_i),
\qquad
\delta_i=\mathbf{1}\{T_i\leq C_i\},
\end{equation}
stored as \texttt{time\_to\_event\_or\_censor\_months} and
\texttt{event}. This is the same $\bigl(T_i^{\mathrm{obs}},\delta_i,X_i\bigr)$
construction defined in Section~\ref{sec:endtoend-construction}.
