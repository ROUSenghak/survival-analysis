...
def generate_clean_world(scenario_id: str, project_root: Path, n_buyers: int,
                          world_seed: int) -> dict[str, pd.DataFrame]:
...
def generate_observed_world(world: dict[str, pd.DataFrame], scenario_id: str, project_root: Path,
                             corruption_seed: int) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def write_pilot_outputs(world: dict[str, pd.DataFrame], observed: pd.DataFrame, corruption_log: pd.DataFrame,
                         ...
                         world_rep: int = 1, corruption_rep: int = 1) -> Path:
    """Phase 7: write the full traceability output tree for one
    scenario/world/corruption replication."""
    project_root = Path(project_root)
    out_dir = (project_root / "data" / "processed" / "synthetic_benchmark" / "v0_1_provisional"
...
def generate_pilot(scenario_id: str, project_root: Path, n_buyers: int | None = None,
                    world_seed: int | None = None, corruption_seed: int | None = None) -> Path:
    ...
    world = generate_clean_world(scenario_id, project_root, n_buyers, world_seed)
    observed, log = generate_observed_world(world, scenario_id, project_root, corruption_seed)
    return write_pilot_outputs(world, observed, log, scenario_id, project_root, world_seed, corruption_seed)
